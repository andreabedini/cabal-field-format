...
import Data.Char (digitToInt, intToDigit, isAlpha, isAlphaNum, isDigit, isSpace, toLower)
...
class (P.CharParsing m, MonadPlus m, MonadFail m) => CabalParsing m where
...
instance MonadFail ParsecParser where
  fail = P.unexpected
...
runParsecParser' v p = Parsec.runParser (unPP p v <* P.eof) []
...
parsecCommaNonEmpty :: (CabalParsing m) => m a -> m (NE.NonEmpty a)
...
parsecLeadingCommaList p = do
  c <- P.optional comma
  case c of
    Nothing -> NE.toList <$> P.sepEndByNonEmpty lp comma <|> pure []
    Just _ -> NE.toList <$> P.sepByNonEmpty lp comma
  where
    lp = p <* P.spaces
...
--
-- @since 3.4.0.0
parsecLeadingCommaNonEmpty :: (CabalParsing m) => m a -> m (NE.NonEmpty a)
    ...
    bounded base bnd =
      ...
      where
        ...
        bounded' dps@(zero : _) bds =
          P.skipSome zero
            *> ([] <$ P.notFollowedBy (P.choice dps) <|> bounded'' dps bds)
            <|> bounded'' dps bds
